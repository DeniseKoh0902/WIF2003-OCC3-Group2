...
import { initializeCalendar, navigateMonth, addWorkout, removeWorkout } from './calendar.js';
...
function initializeApp() {
    ...
    // Notifications.init(); // Assuming Notifications is an IIFE or object with init
...
function updateUserInterface() {
    ...
    if (weeklySummary) { // Add checks in case data is missing
        document.querySelector('.active-time-card .card-value').textContent = weeklySummary.activeTime.toLocaleString();
        document.querySelector('.steps-card .card-value').textContent = weeklySummary.steps.toLocaleString();
        document.querySelector('.calories-card .card-value').textContent = weeklySummary.averageCalories.toLocaleString();
    } else { console.warn("weeklySummary data missing"); }

      if (userData) { // Add checks
        ...
        if(weightEl) weightEl.textContent = userData.weight; else console.warn("weight-value element not found");
        if(heightEl) heightEl.textContent = userData.height; else console.warn("height-value element not found");
        if(ageEl) ageEl.textContent = userData.age; else console.warn("age-value element not found");
        if(bmiValEl) bmiValEl.textContent = userData.bmi; else console.warn("bmi-value element not found");
        if(usernameEl) usernameEl.textContent = `@${userData.username}`; else console.warn("username element not found");
        ...
        } else { console.warn("userData missing"); }
     ...
     if(editBmiBtn) { // Add check
          editBmiBtn.addEventListener('click', function() {
             ...
             if (window.appData && window.appData.userData) {
                 ...
                 console.warn("appData.userData not available for pre-filling BMI form.");
             }
...
// --- Add the throttled resize listener after DOMContentLoaded ---
 document.addEventListener('DOMContentLoaded', () => {
    console.log('CHARMSYNC app initialized');
    ...
    setupEventListeners();
     ...
     // Set up global throttled resize listener
